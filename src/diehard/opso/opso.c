...
//! @brief This file implements the Diehard OPSO test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_opso_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "opso"
#define TEST_DESCRIPTION \
"Overlapping Pairs Sparse Occupancy. Generates 2-letter words from an alphabet \
of 1024 letters, where each letter is formed from 10 consecutive bits. Counts \
missing 2-letter words in an overlapping stream. The number missing should be \
approximately normal with mean 141,909 and sigma 290. WARNING: From the original \
Dieharder suite — this test is marked as suspect due to known issues with sample \
size relative to the word space."
...
#define OPSO_WORD_BITS              20
#define OPSO_TOTAL_WORDS            (1 << OPSO_WORD_BITS)   // 1,048,576
#define OPSO_EXPECTED_MISSING       141909.0
#define OPSO_SIGMA                  290.0

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_opsoprivatedata
{
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_OpsoPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    // Allocate seen array for all possible 20-bit words
    result = STEER_AllocateMemory(OPSO_TOTAL_WORDS * sizeof(uint8_t), (void**)&seen);
    if (result != STEER_RESULT_SUCCESS)
        return result;

    memset(seen, 0, OPSO_TOTAL_WORDS * sizeof(uint8_t));

    // Number of overlapping 20-bit words we can extract
    numWords = totalBits - OPSO_WORD_BITS + 1;

    // Extract overlapping 20-bit words using a sliding window
    // Build the first word from the first 20 bits
    word = 0;
    for (i = 0; i < OPSO_WORD_BITS; i++)
        word = (word << 1) | bitstreamBuffer[i];
    seen[word] = 1;

    // Slide one bit at a time
    for (i = 1; i < numWords; i++)
    {
        word = ((word << 1) | bitstreamBuffer[i + OPSO_WORD_BITS - 1]) & (OPSO_TOTAL_WORDS - 1);
        seen[word] = 1;
    }

    // Count missing words
    missing = 0;
    for (i = 0; i < OPSO_TOTAL_WORDS; i++)
    {
        if (seen[i] == 0)
            missing++;
    }
    ...
    z = ((double)missing - OPSO_EXPECTED_MISSING) / OPSO_SIGMA;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_OpsoPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_OpsoPrivateData), (void**)&privData);
...
{
    ((tDiehard_OpsoPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_OpsoPrivateData* privData = (tDiehard_OpsoPrivateData*)testPrivateData;
    bool passed = false;
