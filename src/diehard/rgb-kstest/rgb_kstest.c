...
//! @brief This file implements the Dieharder RGB Kolmogorov-Smirnov test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_rgb_kstest_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "rgb kstest"
#define TEST_DESCRIPTION \
"A Kolmogorov-Smirnov goodness-of-fit test applied directly to the uniform distribution \
of random values from the generator. Converts the bitstream into uniform [0,1) values \
and tests whether they truly follow a uniform distribution using the KS statistic. \
The test computes the maximum deviation between the empirical CDF and the theoretical \
uniform CDF. NOTE: From the original Dieharder suite -- this test is considered reliable \
and is used internally by Dieharder to validate p-value distributions."
...
typedef struct tdiehard_rgbkstestprivatedata
{
...
static tSTEER_InfoList gReferences = {
    3,
    {
        ...
        "N.V. Smirnov, Table for Estimating the Goodness of Fit, Annals of Math Stat, 1948"
    }
};
...
// =================================================================================================
//  Helper: KS p-value using asymptotic formula
// =================================================================================================
static double KSPvalue(double D, uint64_t N)
{
    ...
    for (k = 1; k <= 100; k++)
    {
        double term = sign * exp(-2.0 * (double)k * (double)k * lambda2);
        sum += term;
        sign = -sign;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_RgbKstestPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t totalBits = privateData->bitstreamLength;
    uint64_t N = totalBits / 32;
    double* values = NULL;
    double Dplus = 0.0;
    double Dminus = 0.0;
    double D;
    ...
    result = STEER_AllocateMemory(N * sizeof(double), (void**)&values);

    if (result == STEER_RESULT_SUCCESS)
    {
        // Fill values: convert 32-bit integers to uniform [0,1) doubles
        for (i = 0; i < N; i++)
        {
            uint32_t raw = ExtractUint32(bitstreamBuffer, i * 32);
            values[i] = ((double)raw + 0.5) / 4294967296.0;
        }

        // Sort
        qsort(values, (size_t)N, sizeof(double), CompareDoubles);

        // Compute D+, D-
        for (i = 0; i < N; i++)
        {
            double Fn = (double)(i + 1) / (double)N;
            double Fn_prev = (double)i / (double)N;
            double dp = Fn - values[i];
            double dm = values[i] - Fn_prev;
        ...
        privateData->ksStatistic = D;
        privateData->probabilityValue = KSPvalue(D, N);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_RgbKstestPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_RgbKstestPrivateData), (void**)&privData);
...
{
    ((tDiehard_RgbKstestPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_RgbKstestPrivateData* privData = (tDiehard_RgbKstestPrivateData*)testPrivateData;
    bool passed = false;
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT,
                STEER_DEFAULT_FLOATING_POINT_PRECISION, privData->ksStatistic);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            "KS statistic",
                                            STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
                                            STEER_JSON_VALUE_DEFAULT_FLOATING_POINT_PRECISION,
