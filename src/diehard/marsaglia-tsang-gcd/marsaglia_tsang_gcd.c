// =================================================================================================
//! @file marsaglia_tsang_gcd.c
//! @author STEER Framework Contributors
//! @brief This file implements the Diehard Marsaglia-Tsang GCD test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_marsaglia_tsang_gcd_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "marsaglia tsang gcd"
#define TEST_DESCRIPTION \
"Generates pairs of random 32-bit integers and computes their GCD using the Euclidean \
algorithm. Tests the distribution of the number of steps k required by the algorithm. \
The step count distribution should follow a known distribution derived from the theory \
of continued fractions. A chi-squared test compares observed vs expected step counts. \
NOTE: From the original Dieharder suite — this test is considered reliable. Many \
congruential random number generators fail this test."
...
#define NUM_PAIRS_DEFAULT           10000
#define NUM_GCD_BINS                7

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_marsagliatsanggcdprivatedata
{
...
tDiehard_MarsagliaTsangGcdPrivateData;
...
static tSTEER_InfoList gReferences = {
    3,
    {
        "George Marsaglia, Wai Wan Tsang, Some Difficult-to-pass Tests of Randomness, 2002",
        "George Marsaglia, DIEHARD: A Battery of Tests of Randomness, 1996",
        "Donald E. Knuth, The Art of Computer Programming, Vol 2, Section 4.5.3"
    }
};
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_MarsagliaTsangGcdPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t totalBits = privateData->bitstreamLength;
    uint64_t maxPairs = totalBits / 64;
    uint64_t numPairs = privateData->numPairs;
    uint64_t observed[NUM_GCD_BINS];
    double expected[NUM_GCD_BINS];
    double gcdProb[NUM_GCD_BINS];
    double sumProb = 0.0;
    double chiSquared = 0.0;
    uint64_t i;
    int d;

    if (maxPairs < numPairs)
        numPairs = maxPairs;

    memset(observed, 0, sizeof(observed));

    // Compute probabilities: P(gcd = d) = 6 / (pi^2 * d^2)
    for (d = 1; d <= 6; d++)
    {
        gcdProb[d - 1] = 6.0 / (M_PI * M_PI * (double)(d * d));
        sumProb += gcdProb[d - 1];
    }
    gcdProb[6] = 1.0 - sumProb;    // P(gcd >= 7)

    // Compute GCDs for each pair
    for (i = 0; i < numPairs; i++)
    {
        uint32_t u = ExtractUint32(bitstreamBuffer, i * 64);
        uint32_t v = ExtractUint32(bitstreamBuffer, i * 64 + 32);
        uint32_t t;
        uint32_t gcd_val;
        int bin;
    ...
    chiSquared = 0.0;
    for (d = 0; d < NUM_GCD_BINS; d++)
    {
        expected[d] = gcdProb[d] * (double)numPairs;
        if (expected[d] > 0.0)
        {
            double diff = (double)observed[d] - expected[d];
            chiSquared += (diff * diff) / expected[d];
        }
    }

    privateData->chiSquared = chiSquared;

    // P-value using upper incomplete gamma function (df = NUM_GCD_BINS - 1 = 6)
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_MarsagliaTsangGcdPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_MarsagliaTsangGcdPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->numPairs = NUM_PAIRS_DEFAULT;
...
{
    ((tDiehard_MarsagliaTsangGcdPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_MarsagliaTsangGcdPrivateData* privData = (tDiehard_MarsagliaTsangGcdPrivateData*)testPrivateData;
    bool passed = false;
