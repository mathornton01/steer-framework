// =================================================================================================
//! @file dab_filltree2.c
//! @author STEER Framework Contributors
//! @brief This file implements the Dieharder DAB Fill Tree 2 test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_dab_filltree2_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "dab filltree2"
#define TEST_DESCRIPTION \
"A refined variant of the Fill Tree test that examines binary tree filling patterns with \
corrected methodology. Uses random bits to traverse a binary tree and tracks which internal \
nodes become 'complete' (both subtrees fully visited). The proportion of complete nodes should \
follow a predictable distribution for random input. A chi-squared test compares observed vs \
expected completion patterns. NOTE: From the original Dieharder suite -- this test is \
considered reliable, improving on the original Fill Tree test."
...
#define DEFAULT_TREE_DEPTH          15

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_dabfilltree2privatedata
{
...
tDiehard_DabFilltree2PrivateData;
...
static tSTEER_InfoList gReferences = {
    3,
    {
        "David Bauer, Dieharder: Additional Tests, 2006",
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_DabFilltree2PrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t D = privateData->treeDepth;
    uint64_t numNodes = ((uint64_t)1 << (D + 1)) - 1;
    uint64_t numLeaves = (uint64_t)1 << D;
    uint64_t totalBits = privateData->bitstreamLength;
    uint64_t numWalks = totalBits / D;
    uint64_t bitPos = 0;
    uint8_t* visited = NULL;
    uint64_t completeCount = 0;
    uint64_t numInternal = numNodes - numLeaves;
    uint64_t i = 0;
    uint64_t j = 0;
    double lambda = 0.0;
    uint64_t maxBin = 0;
    uint64_t numBins = 0;
    uint32_t* leafCount = NULL;
    uint64_t* binCounts = NULL;
    double* binExpected = NULL;
    ...
    // Allocate and initialize node visit tracker
    result = STEER_AllocateMemory(numNodes * sizeof(uint8_t), (void**)&visited);
    if (result != STEER_RESULT_SUCCESS)
    {
        *passed = false;
        return result;
    }
    memset(visited, 0, numNodes * sizeof(uint8_t));

    // Perform walks from root, tracking which directions are taken at each internal node
    for (i = 0; i < numWalks && bitPos + D <= totalBits; i++)
    {
        uint64_t node = 0;
        for (j = 0; j < D; j++)
        {
            uint8_t bit = bitstreamBuffer[bitPos++];
            visited[node] |= (1 << bit);
            node = 2 * node + 1 + bit;
        }
    }

    // Count internal nodes where both children were visited (visited[node] == 3)
    completeCount = 0;
    ...
    STEER_FreeMemory((void**)&visited);

    // Allocate leaf count array
    result = STEER_AllocateMemory(numLeaves * sizeof(uint32_t), (void**)&leafCount);
    if (result != STEER_RESULT_SUCCESS)
    {
    ...
    memset(leafCount, 0, numLeaves * sizeof(uint32_t));

    // Re-walk to count leaf visits
    bitPos = 0;
    for (i = 0; i < numWalks && bitPos + D <= totalBits; i++)
    {
        ...
        {
            uint8_t bit = bitstreamBuffer[bitPos++];
            leafIndex = (leafIndex << 1) | bit;
        }
        leafCount[leafIndex]++;
    }

    // Bin the leaf counts using Poisson distribution
    lambda = (double)numWalks / (double)numLeaves;
    maxBin = (uint64_t)(lambda * 3.0);
    if (maxBin < 5)
        maxBin = 5;
    if (maxBin > 50)
        maxBin = 50;
    ...
    result = STEER_AllocateMemory(numBins * sizeof(uint64_t), (void**)&binCounts);
    if (result != STEER_RESULT_SUCCESS)
    {
        STEER_FreeMemory((void**)&leafCount);
        privateData->chiSquared = 0.0;
        privateData->probabilityValue = 0.0;
        *passed = false;
        return result;
    ...
    result = STEER_AllocateMemory(numBins * sizeof(double), (void**)&binExpected);
    if (result != STEER_RESULT_SUCCESS)
    {
        STEER_FreeMemory((void**)&leafCount);
        STEER_FreeMemory((void**)&binCounts);
        privateData->chiSquared = 0.0;
        privateData->probabilityValue = 0.0;
        *passed = false;
        return result;
    ...
    memset(binCounts, 0, numBins * sizeof(uint64_t));

    for (i = 0; i < numLeaves; i++)
    {
        uint64_t bin = leafCount[i];
        if (bin >= numBins - 1)
            bin = numBins - 1;
        binCounts[bin]++;
    }

    // Expected: Poisson(lambda) probabilities * numLeaves
    poissonSum = 0.0;
    for (i = 0; i < numBins - 1; i++)
    {
        poissonProb = exp(-lambda);
        for (j = 1; j <= i; j++)
            poissonProb *= lambda / (double)j;
        binExpected[i] = poissonProb * (double)numLeaves;
        poissonSum += poissonProb;
    }
    binExpected[numBins - 1] = (1.0 - poissonSum) * (double)numLeaves;
    ...
    for (i = 0; i < numBins; i++)
    {
        if (binExpected[i] >= 5.0)
        {
            diff = (double)binCounts[i] - binExpected[i];
            chiSquared += (diff * diff) / binExpected[i];
            df++;
        }
    }
    if (df > 1)
    ...
    STEER_FreeMemory((void**)&leafCount);
    STEER_FreeMemory((void**)&binCounts);
    STEER_FreeMemory((void**)&binExpected);

    privateData->chiSquared = chiSquared;
    if (df > 0)
        privateData->probabilityValue = cephes_igamc((double)df / 2.0, chiSquared / 2.0);
    else
        privateData->probabilityValue = 0.0;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabFilltree2PrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_DabFilltree2PrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->treeDepth = DEFAULT_TREE_DEPTH;
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, "tree depth") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->treeDepth = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
...
{
    ((tDiehard_DabFilltree2PrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabFilltree2PrivateData* privData = (tDiehard_DabFilltree2PrivateData*)testPrivateData;
    bool passed = false;
