// =================================================================================================
//! @file count_1s_stream.c
//! @author STEER Framework Contributors
//! @brief This file implements the Diehard Count the 1s (Stream) test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_count_1s_stream_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "count 1s stream"
#define TEST_DESCRIPTION \
"Counts the number of 1-bits in each successive byte of the random stream, converts the \
counts to letters (0-2->A, 3->B, 4->C, 5->D, 6-8->E), and counts occurrences of all \
possible 5-letter words. Tests the word frequency distribution against the expected \
multinomial distribution using chi-squared. NOTE: From the original Dieharder suite -- \
...
typedef struct tdiehard_count1sstreamprivatedata
{
...
tDiehard_Count1sStreamPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_Count1sStreamPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t totalBits = privateData->bitstreamLength;
    uint64_t numBytes = totalBits / 8;
    uint64_t numWords = 0;
    uint64_t i = 0;
    uint64_t j = 0;
    ...
    // Allocate letters array
    result = STEER_AllocateMemory(numBytes * sizeof(uint8_t), (void**)&letters);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        // Convert each byte to a letter based on popcount
        for (i = 0; i < numBytes; i++)
        {
            uint8_t byte = 0;
            for (j = 0; j < 8; j++)
                byte = (byte << 1) | bitstreamBuffer[i * 8 + j];
        ...
        if (numBytes >= WORD_LENGTH)
            numWords = numBytes - WORD_LENGTH + 1;
        else
            numWords = 0;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_Count1sStreamPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_Count1sStreamPrivateData), (void**)&privData);
...
{
    ((tDiehard_Count1sStreamPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_Count1sStreamPrivateData* privData = (tDiehard_Count1sStreamPrivateData*)testPrivateData;
    bool passed = false;
