...
//! @brief This file implements the Dieharder RGB Bit Distribution test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_rgb_bitdist_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "rgb bitdist"
#define TEST_DESCRIPTION \
"Tests the distribution of n-bit patterns (n-tuples) in the random bitstream for uniformity. \
For each possible n-bit pattern, the observed count is compared against the expected uniform \
frequency. This is a generalization of the monobit test: at ntuple=1 it tests bit balance, \
at ntuple=2 it tests pair balance, and so on. A chi-squared goodness-of-fit test is applied. \
NOTE: From the original Dieharder suite — this test is considered reliable and very thorough, \
though slower for large ntuple values."
...
#define DEFAULT_NTUPLE              8

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_rgbbitdistprivatedata
{
...
tDiehard_RgbBitdistPrivateData;
...
static tSTEER_InfoList gReferences = {
    3,
    {
        ...
        "Donald E. Knuth, The Art of Computer Programming, Vol 2, 3rd Ed, 1997"
    }
};
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_RgbBitdistPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t n = privateData->ntuple;
    uint64_t numPatterns = (uint64_t)1 << n;
    ...
    numWindows = totalBits - n + 1;

    result = STEER_AllocateMemory(numPatterns * sizeof(uint64_t), (void**)&observed);
    if (result != STEER_RESULT_SUCCESS)
        return result;

    memset(observed, 0, numPatterns * sizeof(uint64_t));
    ...
    expected = (double)numWindows / (double)numPatterns;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_RgbBitdistPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_RgbBitdistPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->ntuple = DEFAULT_NTUPLE;
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, "ntuple") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->ntuple = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
...
{
    ((tDiehard_RgbBitdistPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_RgbBitdistPrivateData* privData = (tDiehard_RgbBitdistPrivateData*)testPrivateData;
    bool passed = false;
