// =================================================================================================
//! @file dab_bytedistrib.c
//! @author STEER Framework Contributors
//! @brief This file implements the Dieharder DAB Byte Distribution test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_dab_bytedistrib_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "dab bytedistrib"
#define TEST_DESCRIPTION \
"Tests the distribution of all 256 possible byte values in the random bitstream. \
Each byte value should occur with approximately equal frequency for a truly random \
source. A chi-squared goodness-of-fit test is applied to the observed frequencies. \
NOTE: From the original Dieharder suite — this test is considered reliable and \
effective for detecting byte-level bias."
...
#define DEGREES_OF_FREEDOM          255

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_dabbytedistribprivatedata
{
...
tDiehard_DabByteDistribPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_DabByteDistribPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    uint64_t observed[NUM_BYTE_VALUES];
    double expected = 0.0;
    ...
    // Convert bitstream buffer into bytes and count occurrences of each byte value
    for (byteIdx = 0; byteIdx < totalBytes; byteIdx++)
    {
        byteVal = 0;
        for (i = 0; i < 8; i++)
            byteVal = (byteVal << 1) | bitstreamBuffer[byteIdx * 8 + i];

        observed[byteVal]++;
    }

    // Expected frequency for each byte value
    expected = (double)totalBytes / 256.0;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabByteDistribPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_DabByteDistribPrivateData), (void**)&privData);
...
{
    ((tDiehard_DabByteDistribPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabByteDistribPrivateData* privData = (tDiehard_DabByteDistribPrivateData*)testPrivateData;
    bool passed = false;
