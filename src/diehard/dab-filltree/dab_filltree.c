...
//! @brief This file implements the Dieharder DAB Fill Tree test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_dab_filltree_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "dab filltree"
#define TEST_DESCRIPTION \
"Tests binary tree filling patterns using random bits. Inserts random bit values into a \
complete binary tree of a given depth and examines the statistical properties of the resulting \
filled tree. For a truly random source, the number of nodes that become 'filled' (both children \
visited) should follow a predictable distribution. A chi-squared test compares the observed fill \
pattern distribution against the expected distribution. WARNING: From the original Dieharder \
suite -- this test is marked as potentially unreliable and results should be interpreted with \
caution."
...
typedef struct tdiehard_dabfilltreeprivatedata
{
...
tDiehard_DabFilltreePrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_DabFilltreePrivateData* privateData,
                ...
                bool* passed)
{
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        // Chi-squared test: each leaf expected = numWalks / numLeaves
        expected = (double)numWalks / (double)numLeaves;

        if (expected > 0.0)
        {
            for (i = 0; i < numLeaves; i++)
            {
                double diff = (double)leafCount[i] - expected;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabFilltreePrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_DabFilltreePrivateData), (void**)&privData);
...
{
    ((tDiehard_DabFilltreePrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabFilltreePrivateData* privData = (tDiehard_DabFilltreePrivateData*)testPrivateData;
    bool passed = false;
