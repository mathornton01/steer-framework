...
//! @brief This file implements the Diehard Craps test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_craps_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "craps"
#define TEST_DESCRIPTION \
"Plays many games of craps using random numbers, counting wins and the number of throws \
per game. The number of wins should be approximately normally distributed with mean \
N*244/495 where N is the number of games. The distribution of throws per game is also \
tested. NOTE: From the original Dieharder suite — this test is considered reliable."
...
#define NUM_GAMES_DEFAULT           200000

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_crapsprivatedata
{
...
// =================================================================================================
//  Helper: roll a single die (1-6) using 32 bits from the bitstream
// =================================================================================================
static int RollDie(uint8_t* buffer, uint64_t* bitPos, uint64_t totalBits)
{
    if (*bitPos + 32 > totalBits)
        return -1;
    uint32_t raw = ExtractUint32(buffer, *bitPos);
    *bitPos += 32;
    return (int)(raw % 6) + 1;
}
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_CrapsPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    uint64_t bitPos = 0;
    uint64_t numGames = privateData->numGames;
    ...
    {
        int die1 = RollDie(bitstreamBuffer, &bitPos, totalBits);
        int die2 = RollDie(bitstreamBuffer, &bitPos, totalBits);
        if (die1 < 0 || die2 < 0)
            break;

        int comeOut = die1 + die2;

        if (comeOut == 7 || comeOut == 11)
        {
            // Natural — win on come-out roll
            wins++;
            gamesPlayed++;
        }
        else if (comeOut == 2 || comeOut == 3 || comeOut == 12)
        {
            // Craps — lose on come-out roll
            gamesPlayed++;
        }
        else
        {
            ...
            while (!resolved)
            {
                die1 = RollDie(bitstreamBuffer, &bitPos, totalBits);
                die2 = RollDie(bitstreamBuffer, &bitPos, totalBits);
                if (die1 < 0 || die2 < 0)
                {
                    resolved = -1;
                    break;
    ...
    // Test win count with z-score
    expectedWins = (double)gamesPlayed * p_win;
    variance = (double)gamesPlayed * p_win * (1.0 - p_win);
    z = ((double)wins - expectedWins) / sqrt(variance);

    privateData->chiSquared = z;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_CrapsPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_CrapsPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->numGames = NUM_GAMES_DEFAULT;
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, "num games") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->numGames = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
...
{
    ((tDiehard_CrapsPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_CrapsPrivateData* privData = (tDiehard_CrapsPrivateData*)testPrivateData;
    bool passed = false;
