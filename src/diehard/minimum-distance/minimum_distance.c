// =================================================================================================
//! @file minimum_distance.c
//! @author STEER Framework Contributors
//! @brief This file implements the Diehard Minimum Distance test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_minimum_distance_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "minimum distance"
#define TEST_DESCRIPTION \
"The Minimum Distance test places 8000 random points in a 10000x10000 square \
and computes the minimum distance d between any pair of points. The square of \
the minimum distance (d^2) should be approximately exponentially distributed \
with mean 0.995. This test is particularly sensitive to generators that fail \
to adequately fill multi-dimensional space."
#define CONFIGURATION_COUNT         1
#define DIEHARD_NAME                "Diehard Statistical Test Battery"

#define MINIMUM_BITSTREAM_COUNT     1
#define MINIMUM_BITSTREAM_LENGTH    524288
#define MINIMUM_SIGNIFICANCE_LEVEL  0.0
#define MAXIMUM_SIGNIFICANCE_LEVEL  1.0

// Minimum distance parameters
#define MINDIST_NUM_POINTS  8000
#define MINDIST_AREA_SIDE   10000.0
#define MINDIST_EXP_MEAN    0.995

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_minimumdistanceprivatedata
{
...
tDiehard_MinimumDistancePrivateData;
...
static tSTEER_InfoList gContributors = {
    1, { STEER_CONTRIBUTOR_GARY_WOODCOCK }
};

static tSTEER_InfoList gMaintainers = {
    2, { STEER_MAINTAINER_ANAMETRIC, STEER_MAINTAINER_SMU_DARWIN_DEASON }
};

static tSTEER_TestInfo gTestInfo = {
    TEST_NAME, DIEHARD_NAME, TEST_DESCRIPTION,
    eSTEER_Complexity_Average, &gReferences, PROGRAM_NAME, PROGRAM_VERSION,
    eSTEER_InputFormat_Bitstream, STEER_REPOSITORY_URI,
    &gAuthors, &gContributors, &gMaintainers, STEER_CONTACT
};

static tSTEER_ParameterInfoList gParameterInfoList = {
    3,
    {
        { STEER_JSON_TAG_BITSTREAM_COUNT, STEER_JSON_VALUE_UNSIGNED_64_BIT_INTEGER,
          NULL, STEER_JSON_VALUE_BITSTREAMS, "1", "1", NULL },
        { STEER_JSON_TAG_BITSTREAM_LENGTH, STEER_JSON_VALUE_UNSIGNED_64_BIT_INTEGER,
          NULL, STEER_JSON_VALUE_BITS, "1000000", "524288", NULL },
        { STEER_JSON_TAG_SIGNIFICANCE_LEVEL, STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
...
static tSTEER_ParametersInfo gParametersInfo = { TEST_NAME, &gParameterInfoList };

// =================================================================================================
//  Helper: extract 32-bit uint from bitstream and convert to double [0, range)
// =================================================================================================
static double ExtractDouble(uint8_t* buffer, uint64_t bitOffset, double range)
{
    uint32_t value = 0;
    int i;
    for (i = 0; i < 32; i++)
        value = (value << 1) | buffer[bitOffset + i];
    return ((double)value / 4294967296.0) * range;
}

// =================================================================================================
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_MinimumDistancePrivateData* privateData,
                ...
                bool* passed)
{
    ...
    numPoints = privateData->bitstreamLength / 64;
    if (numPoints > MINDIST_NUM_POINTS)
        numPoints = MINDIST_NUM_POINTS;

    result = STEER_AllocateMemory(sizeof(double) * numPoints, (void**)&pointX);
    if (result == STEER_RESULT_SUCCESS)
        result = STEER_AllocateMemory(sizeof(double) * numPoints, (void**)&pointY);

    if (result == STEER_RESULT_SUCCESS)
    {
        // Extract random points in [0, 10000) x [0, 10000)
        for (i = 0; i < numPoints; i++)
        {
            pointX[i] = ExtractDouble(bitstreamBuffer, i * 64, MINDIST_AREA_SIDE);
            pointY[i] = ExtractDouble(bitstreamBuffer, i * 64 + 32, MINDIST_AREA_SIDE);
        }

        // Find minimum distance between any pair of points (brute force O(n^2))
        dMin = MINDIST_AREA_SIDE * sqrt(2.0);  // Maximum possible distance
        for (i = 0; i < numPoints; i++)
        {
    ...
    privateData->minDistance = dMin;

    // Compute d^2
    dSquared = dMin * dMin;
    privateData->dSquared = dSquared;

    // P-value = 1.0 - exp(-d^2 / 0.995) — exponential CDF
    privateData->probabilityValue = 1.0 - exp(-dSquared / MINDIST_EXP_MEAN);

    *passed = (privateData->probabilityValue >= privateData->significanceLevel);

    STEER_FreeMemory((void**)&pointX);
    STEER_FreeMemory((void**)&pointY);
...
char* GetTestInfo(void)
{
    ...
    return (result == STEER_RESULT_SUCCESS) ? json : NULL;
}
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_MinimumDistancePrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_MinimumDistancePrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, STEER_JSON_TAG_SIGNIFICANCE_LEVEL) == 0)
            {
                ...
                if (result == STEER_RESULT_SUCCESS && parameters->parameter[i].precision != NULL)
                    result = STEER_ConvertStringToUnsigned32BitInteger(
                        parameters->parameter[i].precision, &(privData->significanceLevelPrecision));
            }
        ...
        if (result == STEER_RESULT_SUCCESS)
            result = STEER_GetMinimumTestCount(privData->significanceLevel, privData->bitstreamCount,
                        &(privData->minimumTestCountRequiredForSignificance),
                        &(privData->predictedPassedTestCount), &(privData->predictedFailedTestCount));
...
// =================================================================================================
uint32_t GetConfigurationCount(void* testPrivateData) { return CONFIGURATION_COUNT; }
...
{
    ((tDiehard_MinimumDistancePrivateData*)testPrivateData)->report = report;
...
//  ExecuteTest
// =================================================================================================
int32_t ExecuteTest(void* testPrivateData, const char* bitstreamId,
                    uint8_t* buffer, uint64_t bufferSizeInBytes,
                    uint64_t bytesInBuffer, uint64_t numZeros, uint64_t numOnes)
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_MinimumDistancePrivateData* privData = (tDiehard_MinimumDistancePrivateData*)testPrivateData;
    bool passed = false;
    char calculationStr[STEER_STRING_MAX_LENGTH] = { 0 };
    char criterionStr[STEER_STRING_MAX_LENGTH] = { 0 };
    uint64_t testId = strtoull(bitstreamId, NULL, 0) - 1;
    ...
    if (result == STEER_RESULT_SUCCESS) {
        ...
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                    STEER_JSON_TAG_ONES, STEER_JSON_VALUE_UNSIGNED_64_BIT_INTEGER,
                    NULL, STEER_JSON_VALUE_BITS, calculationStr);
    }
    if (result == STEER_RESULT_SUCCESS) {
        sprintf(calculationStr, "%" PRIu64 "", numZeros);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                    STEER_JSON_TAG_ZEROS, STEER_JSON_VALUE_UNSIGNED_64_BIT_INTEGER,
                    NULL, STEER_JSON_VALUE_BITS, calculationStr);
    }
    if (result == STEER_RESULT_SUCCESS) {
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT,
                STEER_DEFAULT_FLOATING_POINT_PRECISION, privData->minDistance);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                    "minimum distance", STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
                    STEER_JSON_VALUE_DEFAULT_FLOATING_POINT_PRECISION, NULL, calculationStr);
    }
    if (result == STEER_RESULT_SUCCESS) {
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT,
                STEER_DEFAULT_FLOATING_POINT_PRECISION, privData->dSquared);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                    "d squared", STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
                    STEER_JSON_VALUE_DEFAULT_FLOATING_POINT_PRECISION, NULL, calculationStr);
    }
    if (result == STEER_RESULT_SUCCESS) {
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT,
    ...
    if (result == STEER_RESULT_SUCCESS) {
        sprintf(criterionStr, "%s of %.*f >= %s of %.*f",
                ...
                privData->significanceLevelPrecision, privData->significanceLevel);
    ...
    if (result == STEER_RESULT_SUCCESS) {
        ...
        if (result == STEER_RESULT_SUCCESS) {
            ...
            if (eval_passed) privData->configurationState[0].testsPassed++;
            else privData->configurationState[0].testsFailed++;
        }
    }
...
int32_t FinalizeTest(void** testPrivateData, uint64_t suppliedNumberOfBitstreams)
{
    ...
    if (privData != NULL)
    {
        ...
        uint64_t proportionThresholdMinimum = 0, proportionThresholdMaximum = 0;

        result = STEER_NistStsAddRequiredMetricsToConfiguration(privData->report, 0,
                    suppliedNumberOfBitstreams, privData->minimumTestCountRequiredForSignificance,
                    privData->configurationState[0].testsPassed, privData->predictedPassedTestCount,
        ...
        if (result == STEER_RESULT_SUCCESS)
            result = STEER_NistStsAddMetricsToConfiguration(privData->report, 0, false,
                        ...
                        &probabilityValueUniformity, &proportionThresholdMinimum, &proportionThresholdMaximum);
        if (result == STEER_RESULT_SUCCESS)
            result = STEER_AddConfusionMatrixMetricsToConfiguration(privData->report, 0,
                        privData->minimumTestCountRequiredForSignificance,
                        privData->configurationState[0].testsRun, privData->configurationState[0].testsPassed,
                        privData->configurationState[0].testsFailed,
                        privData->predictedPassedTestCount, privData->predictedFailedTestCount);
        if (result == STEER_RESULT_SUCCESS)
            result = STEER_NistStsAddRequiredCriterionToConfiguration(privData->report, 0,
                        suppliedNumberOfBitstreams, privData->configurationState[0].testsPassed,
                        privData->significanceLevel, privData->significanceLevelPrecision,
                        privData->minimumTestCountRequiredForSignificance);
        if (result == STEER_RESULT_SUCCESS)
...
{
    return STEER_Run(PROGRAM_NAME, argc, argv, GetTestInfo, GetParametersInfo,
                     InitTest, GetConfigurationCount, SetReport, ExecuteTest, FinalizeTest);
}
