...
//! @brief This file implements the Diehard Parking Lot test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_parking_lot_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "parking lot"
#define TEST_DESCRIPTION \
"The Parking Lot test randomly attempts to park 12000 unit-diameter circles in a \
100x100 square. A circle is successfully parked if it does not overlap with any \
previously parked circle. The number of successfully parked circles k should be \
approximately normally distributed with mean 3523 and standard deviation 21.9. \
This test is sensitive to generators with insufficient dimensionality or \
...
#define MINIMUM_BITSTREAM_LENGTH    786432
#define MINIMUM_SIGNIFICANCE_LEVEL  0.0
#define MAXIMUM_SIGNIFICANCE_LEVEL  1.0

// Parking lot parameters
#define PARK_ATTEMPTS   12000
#define PARK_AREA       100.0
#define PARK_RADIUS     1.0
#define PARK_EXPECTED   3523.0
#define PARK_STDDEV     21.9

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_parkinglotprivatedata
{
...
tDiehard_ParkingLotPrivateData;
...
static tSTEER_ParameterInfoList gParameterInfoList = {
    3,
    {
        ...
        { STEER_JSON_TAG_BITSTREAM_LENGTH, STEER_JSON_VALUE_UNSIGNED_64_BIT_INTEGER,
          NULL, STEER_JSON_VALUE_BITS, "1000000", "786432", NULL },
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_ParkingLotPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    maxAttempts = privateData->bitstreamLength / 64;
    if (maxAttempts > PARK_ATTEMPTS)
        maxAttempts = PARK_ATTEMPTS;

    result = STEER_AllocateMemory(sizeof(double) * maxAttempts, (void**)&parkedX);
    if (result == STEER_RESULT_SUCCESS)
        result = STEER_AllocateMemory(sizeof(double) * maxAttempts, (void**)&parkedY);

    if (result == STEER_RESULT_SUCCESS)
    {
        for (attempt = 0; attempt < (int)maxAttempts; attempt++)
        {
            double x = ExtractDouble(bitstreamBuffer, (uint64_t)attempt * 64, PARK_AREA);
            double y = ExtractDouble(bitstreamBuffer, (uint64_t)attempt * 64 + 32, PARK_AREA);
            bool canPark = true;
            int j;

            // Check against all previously parked circles
            for (j = 0; j < parked; j++)
            {
                double dx = x - parkedX[j];
                double dy = y - parkedY[j];
                if (dx * dx + dy * dy < 4.0 * PARK_RADIUS * PARK_RADIUS)
                {
                    canPark = false;
                    break;
                }
    ...
    privateData->parkedCount = parked;

    // Z-score: (observed - expected) / stddev
    z = ((double)parked - PARK_EXPECTED) / PARK_STDDEV;
    privateData->zScore = z;

    // P-value from normal CDF: use erfc for two-sided
    privateData->probabilityValue = erfc(fabs(z) / sqrt(2.0));

    *passed = (privateData->probabilityValue >= privateData->significanceLevel);

    STEER_FreeMemory((void**)&parkedX);
    STEER_FreeMemory((void**)&parkedY);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_ParkingLotPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_ParkingLotPrivateData), (void**)&privData);
...
{
    ((tDiehard_ParkingLotPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_ParkingLotPrivateData* privData = (tDiehard_ParkingLotPrivateData*)testPrivateData;
    bool passed = false;
    ...
    if (result == STEER_RESULT_SUCCESS) {
        sprintf(calculationStr, "%d", privData->parkedCount);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                    "parked circles", STEER_JSON_VALUE_SIGNED_32_BIT_INTEGER,
