// =================================================================================================
//! @file overlapping_permutations.c
//! @author STEER Framework Contributors
//! @brief This file implements the Diehard Overlapping Permutations test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_overlapping_permutations_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "overlapping permutations"
#define TEST_DESCRIPTION \
"The Overlapping Permutations test (OPERM5) examines sequences of 5 consecutive \
random numbers and determines their relative ordering (permutation rank). There are \
5! = 120 possible permutations. The test counts the frequency of each permutation \
across overlapping windows and uses a chi-squared test to compare against the expected \
uniform distribution. This test is particularly effective at detecting generators \
with short-range correlations."
#define CONFIGURATION_COUNT         1
#define DIEHARD_NAME                "Diehard Statistical Test Battery"

#define MINIMUM_BITSTREAM_COUNT     1
#define MINIMUM_BITSTREAM_LENGTH    32768
#define MINIMUM_SIGNIFICANCE_LEVEL  0.0
#define MAXIMUM_SIGNIFICANCE_LEVEL  1.0

// Overlapping permutations parameters
#define OPERM_WINDOW        5           // Window size
#define OPERM_NUM_PERMS     120         // 5! = 120 possible permutations
#define OPERM_BITS_PER_VAL  32          // Bits per random value
#define OPERM_DF            119         // Degrees of freedom = 120 - 1

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_overlappingpermutationsprivatedata
{
...
tDiehard_OverlappingPermutationsPrivateData;
...
// =================================================================================================
//  Helper: extract a 32-bit unsigned integer from the bitstream at a given bit offset
// =================================================================================================
static uint32_t ExtractBits(uint8_t* buffer, uint64_t bitOffset, int numBits)
{
    ...
    int i;
    for (i = 0; i < numBits; i++)
        value = (value << 1) | ((buffer[(bitOffset + i) / 8] >> (7 - ((bitOffset + i) % 8))) & 1);
    return value;
}

// =================================================================================================
//  Helper: compute permutation rank of 5 values using the Lehmer code
// =================================================================================================
static int PermRank5(double* v)
{
    int lehmer[OPERM_WINDOW];
    int i, j, rank;

    for (i = 0; i < OPERM_WINDOW; i++)
    {
        lehmer[i] = 0;
        for (j = i + 1; j < OPERM_WINDOW; j++)
        {
            if (v[j] < v[i])
                lehmer[i]++;
        }
    ...
    // rank = lehmer[0]*4! + lehmer[1]*3! + lehmer[2]*2! + lehmer[3]*1! + lehmer[4]*0!
    // Note: lehmer[4] is always 0, so we omit it
    rank = lehmer[0] * 24 + lehmer[1] * 6 + lehmer[2] * 2 + lehmer[3] * 1;
    return rank;
}
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_OverlappingPermutationsPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    int observed[OPERM_NUM_PERMS];
    double expected = 0.0;
    ...
    // Determine how many 32-bit values we can extract from the bitstream
    numValues = (int)(privateData->bitstreamLength / OPERM_BITS_PER_VAL);

    // Need at least OPERM_WINDOW values to form one window
    if (numValues < OPERM_WINDOW)
    {
        *passed = false;
        return STEER_RESULT_SUCCESS;
    }

    // Allocate array for the double values
    result = STEER_AllocateMemory((size_t)numValues * sizeof(double), (void**)&values);
    if (result != STEER_RESULT_SUCCESS)
        return result;

    // Extract random values from the bitstream (32 bits each, convert to double in [0,1))
    for (i = 0; i < numValues; i++)
    {
        uint32_t bits = ExtractBits(bitstreamBuffer, (uint64_t)i * OPERM_BITS_PER_VAL, OPERM_BITS_PER_VAL);
        values[i] = (double)bits / 4294967296.0;    // divide by 2^32
    }

    // Count overlapping windows of 5
    numWindows = numValues - OPERM_WINDOW + 1;
    for (i = 0; i < numWindows; i++)
    {
        int rank = PermRank5(&values[i]);
        observed[rank]++;
    }

    // Expected frequency for each permutation rank
    expected = (double)numWindows / (double)OPERM_NUM_PERMS;

    // Compute chi-squared statistic
    chiSquared = 0.0;
    for (i = 0; i < OPERM_NUM_PERMS; i++)
    {
    ...
    privateData->numWindows = (int32_t)numWindows;

    // P-value using the incomplete gamma function: df = 119
    privateData->probabilityValue = cephes_igamc((double)OPERM_DF / 2.0, chiSquared / 2.0);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_OverlappingPermutationsPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_OverlappingPermutationsPrivateData), (void**)&privData);
...
{
    ((tDiehard_OverlappingPermutationsPrivateData*)testPrivateData)->report = report;
    return STEER_RESULT_SUCCESS;
}
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_OverlappingPermutationsPrivateData* privData = (tDiehard_OverlappingPermutationsPrivateData*)testPrivateData;
    bool passed = false;
    ...
    // Add calculations: number of windows
    if (result == STEER_RESULT_SUCCESS)
    {
        memset(calculationStr, 0, STEER_STRING_MAX_LENGTH);
        sprintf(calculationStr, "%d", privData->numWindows);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            "number of windows",
