...
//! @brief This file implements the Diehard OQSO test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_oqso_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "oqso"
#define TEST_DESCRIPTION \
"Overlapping Quadruples Sparse Occupancy. Generates 4-letter words from an alphabet \
of 32 letters, where each letter is formed from 5 consecutive bits. Counts missing \
4-letter words in an overlapping stream. The number missing should be approximately \
normal with mean 141,909 and sigma 295. WARNING: From the original Dieharder suite \
— this test is marked as suspect due to known issues with sample size."
...
#define OQSO_WORD_BITS              20
#define OQSO_TOTAL_WORDS            (1 << OQSO_WORD_BITS)   // 1,048,576
#define OQSO_EXPECTED_MISSING       141909.0
#define OQSO_SIGMA                  295.0

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_oqsoprivatedata
{
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_OqsoPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    result = STEER_AllocateMemory(OQSO_TOTAL_WORDS * sizeof(uint8_t), (void**)&seen);
    if (result != STEER_RESULT_SUCCESS)
        return result;

    memset(seen, 0, OQSO_TOTAL_WORDS * sizeof(uint8_t));

    // Number of overlapping 20-bit words we can extract
    numWords = totalBits - OQSO_WORD_BITS + 1;

    // Extract overlapping 20-bit words using a sliding window
    // Build the first word from the first 20 bits
    word = 0;
    for (i = 0; i < OQSO_WORD_BITS; i++)
    ...
    {
        word = ((word << 1) | bitstreamBuffer[i + OQSO_WORD_BITS - 1]) & (OQSO_TOTAL_WORDS - 1);
        seen[word] = 1;
    }

    // Count missing words
    missing = 0;
    for (i = 0; i < OQSO_TOTAL_WORDS; i++)
    {
        if (seen[i] == 0)
            missing++;
    }
    ...
    z = ((double)missing - OQSO_EXPECTED_MISSING) / OQSO_SIGMA;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_OqsoPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_OqsoPrivateData), (void**)&privData);
...
{
    ((tDiehard_OqsoPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_OqsoPrivateData* privData = (tDiehard_OqsoPrivateData*)testPrivateData;
    bool passed = false;
