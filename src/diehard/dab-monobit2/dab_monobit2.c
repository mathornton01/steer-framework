...
//! @brief This file implements the Dieharder DAB Monobit 2 test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_dab_monobit2_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "dab monobit2"
#define TEST_DESCRIPTION \
"An enhanced monobit test that divides the bitstream into non-overlapping blocks and tests \
the balance of 0s and 1s within each block. Unlike the simple monobit test which checks \
global balance, this test detects localized bit bias within individual blocks. A chi-squared \
test is applied to the distribution of per-block bit sums. NOTE: From the original Dieharder \
suite — this test is rated as the single most effective test in the entire Dieharder battery \
for detecting random number generator weaknesses."
#define CONFIGURATION_COUNT         1
#define DIEHARD_NAME                "Dieharder Statistical Test Battery"

#define MINIMUM_BITSTREAM_COUNT     1
#define MINIMUM_BITSTREAM_LENGTH    1048576
...
typedef struct tdiehard_dabmonobit2privatedata
{
...
tDiehard_DabMonobit2PrivateData;
...
static tSTEER_InfoList gReferences = {
    3,
    {
        ...
        "NIST SP 800-22, A Statistical Test Suite for Random Number Generators, 2010"
    }
};
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_DabMonobit2PrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t totalBits = privateData->bitstreamLength;
    uint64_t B = privateData->blockSize;
    uint64_t numBlocks = totalBits / B;
    uint64_t bitPos = 0;
    uint64_t i;
    uint64_t j;
    ...
    // For each block, count the number of 1s
    // The expected number of 1s per block = B/2
    // Variance = B/4
    // Test: sum of ((S_i - B/2)^2 / (B/4)) should be chi-squared with numBlocks df
    expectedOnes = (double)B / 2.0;
    variance = (double)B / 4.0;
    ...
    for (i = 0; i < numBlocks; i++)
    {
        ...
        for (j = 0; j < B; j++)
        {
            onesCount += bitstreamBuffer[bitPos++];
        }
        diff = (double)onesCount - expectedOnes;
        chiSquared += (diff * diff) / variance;
    }

    privateData->chiSquared = chiSquared;

    // Degrees of freedom = numBlocks
    if (numBlocks > 0)
        privateData->probabilityValue = cephes_igamc((double)numBlocks / 2.0, chiSquared / 2.0);
    else
        privateData->probabilityValue = 0.0;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabMonobit2PrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_DabMonobit2PrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->blockSize = DEFAULT_BLOCK_SIZE;
...
{
    ((tDiehard_DabMonobit2PrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DabMonobit2PrivateData* privData = (tDiehard_DabMonobit2PrivateData*)testPrivateData;
    bool passed = false;
