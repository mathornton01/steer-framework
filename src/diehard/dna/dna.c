...
//! @brief This file implements the Diehard DNA test for the STEER framework.
...
#define PROGRAM_NAME        "diehard_dna_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "dna"
#define TEST_DESCRIPTION \
"Considers 10-letter words from an alphabet of 4 letters (C, G, A, T), where each \
letter is formed from 2 consecutive bits. Counts missing 10-letter words in an \
overlapping stream. The number missing should be approximately normal with mean \
141,909 and sigma 339. WARNING: From the original Dieharder suite — this test is \
marked as suspect due to known issues with sample size."
...
#define DNA_WORD_BITS               20
#define DNA_TOTAL_WORDS             (1 << DNA_WORD_BITS)    // 1,048,576
#define DNA_EXPECTED_MISSING        141909.0
#define DNA_SIGMA                   339.0

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct tdiehard_dnaprivatedata
{
...
//  RunTest
// =================================================================================================
int32_t RunTest(tDiehard_DnaPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    result = STEER_AllocateMemory(DNA_TOTAL_WORDS * sizeof(uint8_t), (void**)&seen);
    if (result != STEER_RESULT_SUCCESS)
        return result;

    memset(seen, 0, DNA_TOTAL_WORDS * sizeof(uint8_t));

    // Number of overlapping 20-bit words we can extract
    numWords = totalBits - DNA_WORD_BITS + 1;

    // Extract overlapping 20-bit words using a sliding window
    // Build the first word from the first 20 bits
    word = 0;
    for (i = 0; i < DNA_WORD_BITS; i++)
    ...
    {
        word = ((word << 1) | bitstreamBuffer[i + DNA_WORD_BITS - 1]) & (DNA_TOTAL_WORDS - 1);
        seen[word] = 1;
    }

    // Count missing words
    missing = 0;
    for (i = 0; i < DNA_TOTAL_WORDS; i++)
    {
        if (seen[i] == 0)
            missing++;
    }
    ...
    z = ((double)missing - DNA_EXPECTED_MISSING) / DNA_SIGMA;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DnaPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tDiehard_DnaPrivateData), (void**)&privData);
...
{
    ((tDiehard_DnaPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tDiehard_DnaPrivateData* privData = (tDiehard_DnaPrivateData*)testPrivateData;
    bool passed = false;
