...
//! @brief This file implements the TestU01 Savir2 test for the STEER framework.
...
#define PROGRAM_NAME        "testu01_savir2_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "savir2"
#define TEST_DESCRIPTION \
"The Savir2 test uses Savir's algorithm to generate random integers in {0,...,m-1} using \
a variable number of random bits. Starting with range [0, m), at each step the range is \
halved by consuming one random bit to pick the upper or lower half. The number of bits \
consumed per generated integer is tracked. For m=100, the expected bits per integer is \
approximately log2(100) = 6.6439. Over n trials, the average bits consumed is computed \
and a Z-test is applied. This test detects generators whose bit patterns produce biased \
outcomes in rejection-sampling-like algorithms."
...
// Savir2 parameters
#define SAVIR2_M                    100     // Target range [0, m)
#define SAVIR2_N_TRIALS             5000    // Number of trials
#define SAVIR2_EXPECTED_BITS        6.6439  // Expected bits per integer for m=100
#define SAVIR2_VARIANCE             0.6     // Empirical variance
#define SAVIR2_GROUP_SIZE_DEFAULT   30      // Number of values per group

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_savir2privatedata
{
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_Savir2PrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t bitPos = 0;
    uint64_t totalBitsConsumed = 0;
    uint64_t trial;
    uint64_t completedTrials = 0;
    double avgBits;
    double zScore;

    for (trial = 0; trial < SAVIR2_N_TRIALS && bitPos < privateData->bitstreamLength; trial++)
    {
        // Savir's algorithm: generate a random integer in [0, groupSize)
        int lo = 0;
        int hi = (int)privateData->groupSize;
        int bitsUsed = 0;

        while (hi - lo > 1)
        {
            if (bitPos >= privateData->bitstreamLength)
                break;
        ...
        if (hi - lo <= 1)
        {
            totalBitsConsumed += bitsUsed;
            completedTrials++;
        }
        else
        {
            break;  // Ran out of bits mid-trial
        }
    }
    ...
    // Compute average bits consumed per integer
    avgBits = (double)totalBitsConsumed / (double)completedTrials;

    // Z-test
    zScore = (avgBits - SAVIR2_EXPECTED_BITS) / sqrt(SAVIR2_VARIANCE / (double)completedTrials);

    // Store Z^2 for reporting consistency
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_Savir2PrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_Savir2PrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->groupSize = SAVIR2_GROUP_SIZE_DEFAULT;
...
{
    ((tTestU01_Savir2PrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_Savir2PrivateData* privData = (tTestU01_Savir2PrivateData*)testPrivateData;
    bool passed = false;
