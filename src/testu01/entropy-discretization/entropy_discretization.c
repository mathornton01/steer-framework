// =================================================================================================
//! @file entropy_discretization.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Entropy Discretization test (sentrop_EntropyDisc) for the
//!        STEER framework.
...
#define PROGRAM_NAME        "testu01_entropy_discretization_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "entropy discretization"
#define TEST_DESCRIPTION \
"The Entropy Discretization test (sentrop_EntropyDisc) divides the bitstream into non-overlapping \
blocks of L bits, treating each block as an L-bit integer. It computes the sample entropy of the \
resulting sequence of integers: H = -sum(p_i * log2(p_i)) where p_i is the observed frequency of \
each possible L-bit pattern. Under the null hypothesis, the entropy should be close to L (maximum \
entropy). The test standardizes the deviation from the expected entropy to a z-score and computes \
a two-sided p-value."
...
// Entropy discretization test parameters
#define ENTROPY_DISC_BLOCK_SIZE_DEFAULT  8

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_entropydiscretizationprivatedata
{
...
tTestU01_EntropyDiscretizationPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_EntropyDiscretizationPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t blockSize = privateData->blockSize;
    uint64_t numPatterns = 1ULL << blockSize;
    uint64_t numBlocks = privateData->bitstreamLength / blockSize;
    ...
    if (numBlocks < numPatterns)
    {
        *passed = false;
        return result;
    }

    result = STEER_AllocateMemory(numPatterns * sizeof(uint64_t), (void**)&counts);
    if (result == STEER_RESULT_SUCCESS)
    {
        memset(counts, 0, numPatterns * sizeof(uint64_t));
        ...
        for (b = 0; b < numBlocks; b++)
        {
            ...
            for (i = 0; i < blockSize; i++)
                val = (val << 1) | bitstreamBuffer[b * blockSize + i];
            counts[val]++;
        }

        // Compute sample entropy in bits
        entropy = 0.0;
        for (i = 0; i < numPatterns; i++)
        {
            if (counts[i] > 0)
            {
                double p = (double)counts[i] / (double)numBlocks;
                entropy -= p * log2(p);
            }
        }

        privateData->entropy = entropy;

        // Expected entropy with Miller-Madow bias correction
        // E[H_bits] = H_true - (m - 1) / (2 * n * ln(2))
        // where m = numPatterns, n = numBlocks, H_true = blockSize (uniform)
        expectedEntropy = (double)blockSize - (double)(numPatterns - 1) / (2.0 * (double)numBlocks * log(2.0));

        // Variance approximation under H0 (uniform distribution)
        // Var[H] ~ (numPatterns - 1) / (2 * numBlocks^2 * ln(2)^2)
        variance = (double)(numPatterns - 1) / (2.0 * (double)numBlocks * (double)numBlocks * log(2.0) * log(2.0));
        if (variance < 1e-30)
            variance = 1e-30;

        // Standardized z-score
        z = (entropy - expectedEntropy) / sqrt(variance);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_EntropyDiscretizationPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_EntropyDiscretizationPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->blockSize = ENTROPY_DISC_BLOCK_SIZE_DEFAULT;
...
{
    ((tTestU01_EntropyDiscretizationPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_EntropyDiscretizationPrivateData* privData = (tTestU01_EntropyDiscretizationPrivateData*)testPrivateData;
    bool passed = false;
    ...
    // Add calculations: entropy
    if (result == STEER_RESULT_SUCCESS)
    {
        memset(calculationStr, 0, STEER_STRING_MAX_LENGTH);
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT,
                STEER_DEFAULT_FLOATING_POINT_PRECISION, privData->entropy);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            "entropy",
                                            STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
                                            STEER_JSON_VALUE_DEFAULT_FLOATING_POINT_PRECISION,
