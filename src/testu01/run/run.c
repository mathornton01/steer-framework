...
//! @brief This file implements the TestU01 Run test (sknuth_Run) for the STEER framework.
...
#define PROGRAM_NAME        "testu01_run_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "run"
#define TEST_DESCRIPTION \
"The Run test (sknuth_Run) converts the bitstream to a sequence of uniform [0,1) values, \
then counts 'runs up' — maximal subsequences of consecutive increasing values. The number \
of runs of each length 1..5 and >=6 are tallied and compared against the expected distribution \
using a chi-squared test. This test detects failures in the independence structure of the \
generator by examining monotone trends in consecutive outputs."
...
// Run test parameters
#define RUN_BITS_PER_VALUE      32      // Bits per uniform value
#define RUN_MAX_LENGTH_DEFAULT  6       // Categories: lengths 1..5 and >=6

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_runprivatedata
{
    ...
    uint64_t                    maxRunLength;
}
tTestU01_RunPrivateData;
...
// =================================================================================================
//  Helper: extract 32-bit unsigned integer from bitstream at given bit offset
// =================================================================================================
static double ExtractUniform(uint8_t* buffer, uint64_t bitOffset)
{
    uint32_t value = 0;
    int i;
    for (i = 0; i < RUN_BITS_PER_VALUE; i++)
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_RunPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    uint64_t i;
    int runLength;
    uint64_t maxRunLength = privateData->maxRunLength;
    uint64_t runCategories = maxRunLength;
    uint64_t runDf = maxRunLength - 1;

    // Dynamically allocate observed and expected arrays
    result = STEER_AllocateMemory(runCategories * sizeof(uint64_t), (void**)&observed);
    if (result != STEER_RESULT_SUCCESS) return result;

    result = STEER_AllocateMemory(runCategories * sizeof(double), (void**)&expected);
    ...
    memset(observed, 0, runCategories * sizeof(uint64_t));

    // Number of uniform values we can extract
    n = privateData->bitstreamLength / RUN_BITS_PER_VALUE;
    if (n < 2)
    {
        *passed = false;
        STEER_FreeMemory((void**)&observed);
    ...
    // Count runs up: a run is a maximal increasing subsequence
    runLength = 1;
    for (i = 1; i < n; i++)
    {
        double prev = ExtractUniform(bitstreamBuffer, (i - 1) * RUN_BITS_PER_VALUE);
        double curr = ExtractUniform(bitstreamBuffer, i * RUN_BITS_PER_VALUE);
        ...
        {
            // End of run — tally it
            int cat = ((uint64_t)runLength >= maxRunLength) ? (runCategories - 1) : (runLength - 1);
            observed[cat]++;
            totalRuns++;
            runLength = 1;
        }
    ...
    // Expected probabilities for run-up of length k:
    // P(run of length k) = 1/k! - 1/(k+1)! for k = 1..(maxRunLength-1)
    // P(run of length >= maxRunLength) = 1/maxRunLength!
    {
        double factorials[14];
        factorials[0] = 1.0;
        for (i = 1; i <= maxRunLength + 1 && i <= 13; i++)
            factorials[i] = factorials[i - 1] * (double)i;

        for (i = 0; i < runCategories - 1; i++)
            expected[i] = (double)totalRuns * (1.0 / factorials[i + 1] - 1.0 / factorials[i + 2]);

        expected[runCategories - 1] = (double)totalRuns / factorials[maxRunLength];
    }

    // Chi-squared statistic
    chiSquared = 0.0;
    for (i = 0; i < runCategories; i++)
    {
        if (expected[i] > 0.0)
        {
    ...
    privateData->probabilityValue = cephes_igamc((double)runDf / 2.0,
                                                  chiSquared / 2.0);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_RunPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_RunPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->maxRunLength = RUN_MAX_LENGTH_DEFAULT;
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, "max run length") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->maxRunLength = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
...
{
    ((tTestU01_RunPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_RunPrivateData* privData = (tTestU01_RunPrivateData*)testPrivateData;
    bool passed = false;
