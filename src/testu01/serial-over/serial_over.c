...
//! @brief This file implements the TestU01 Serial Over test for the STEER framework.
...
#define PROGRAM_NAME        "testu01_serial_over_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "serial over"
#define TEST_DESCRIPTION \
"The Serial Over test divides the bitstream into overlapping d-tuples of s-bit values \
and counts the frequency of each d-tuple in {0,...,2^s-1}^d. A chi-squared test is then \
applied to these frequencies against the expected uniform distribution. This test detects \
deviations from uniformity in overlapping serial patterns of the generator output."
...
// Serial Over parameter defaults
#define SERIAL_D_DEFAULT        2       // Dimension (d-tuples)
#define SERIAL_S_DEFAULT        4       // Bits per value

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_serialoverprivatedata
{
...
tTestU01_SerialOverPrivateData;
...
// =================================================================================================
//  Helper: extract s-bit value from bitstream at given bit offset
// =================================================================================================
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_SerialOverPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    // Compute derived values from configurable parameters
    uint64_t d = privateData->dimension;
    uint64_t s = privateData->bitsPerValue;
    uint64_t k = 1ULL << s;          // 2^s
    uint64_t cats = 1;
    uint64_t df;
    uint64_t dim;
    for (dim = 0; dim < d; dim++)
    ...
    // Dynamically allocate observed array
    result = STEER_AllocateMemory(cats * sizeof(uint64_t), (void**)&observed);
    if (result != STEER_RESULT_SUCCESS)
        return result;
    memset(observed, 0, cats * sizeof(uint64_t));

    // Number of overlapping d-tuples we can extract
    // Each value requires s bits; a d-tuple requires d values
    // Overlapping means we advance by s bits each step
    n = (privateData->bitstreamLength / s) - (d - 1);

    // Count frequency of each d-tuple
    for (i = 0; i < n; i++)
    {
        uint32_t index = 0;
        uint64_t dd;
        for (dd = 0; dd < d; dd++)
        {
            uint32_t val = ExtractBits(bitstreamBuffer,
                                       (i + dd) * s,
                                       (int)s);
            index = index * (uint32_t)k + val;
        }
        observed[index]++;
    }

    // Expected count per category
    expected = (double)n / (double)cats;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_SerialOverPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_SerialOverPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        // Set defaults
        privData->dimension = SERIAL_D_DEFAULT;
        privData->bitsPerValue = SERIAL_S_DEFAULT;
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, "bits per value") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->bitsPerValue = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
...
{
    ((tTestU01_SerialOverPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_SerialOverPrivateData* privData = (tTestU01_SerialOverPrivateData*)testPrivateData;
    bool passed = false;
