// =================================================================================================
//! @file longest_head_run.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Longest Head Run test (sstring_LongestHeadRun) for
//!        the STEER framework.
...
#define PROGRAM_NAME        "testu01_longest_head_run_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "longest head run"
#define TEST_DESCRIPTION \
"The Longest Head Run test (sstring_LongestHeadRun) divides the bitstream into blocks and \
finds the longest run of consecutive 1-bits (head run) within each block. Under the null \
hypothesis, the expected length of the longest run of 1s in a block of n bits is approximately \
log2(n). The test collects the longest head run from multiple blocks and tests whether the \
distribution matches the expected Gumbel-like distribution using a chi-squared test with \
binned categories."
...
// Longest head run parameters
#define BLOCK_SIZE_DEFAULT          1000
#define BLOCK_SIZE_MIN              100
#define BLOCK_SIZE_MAX              100000
#define LHR_NUM_BINS                6
#define LHR_DF                      5
...
typedef struct ttestu01_longestheadrunprivatedata
{
...
tTestU01_LongestHeadRunPrivateData;
...
// =================================================================================================
//  Helper: find the longest run of consecutive 1-bits in a block
// =================================================================================================
static uint64_t FindLongestRun(uint8_t* buffer, uint64_t bitOffset, uint64_t blockSize)
{
    uint64_t longest = 0;
    uint64_t current = 0;
    uint64_t i;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_LongestHeadRunPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t observed[LHR_NUM_BINS];
    double expected[LHR_NUM_BINS];
    double expectedProb[LHR_NUM_BINS];
    uint64_t numBlocks;
    uint64_t blockSize = privateData->blockSize;
    double expectedMax;
    int expectedMaxInt;
    ...
    numBlocks = privateData->bitstreamLength / blockSize;
    if (numBlocks < 1)
    {
        *passed = false;
        return result;
    ...
    // Expected longest run length: approximately log2(blockSize)
    expectedMax = log2((double)blockSize);
    expectedMaxInt = (int)floor(expectedMax);

    // For each block, find the longest run of 1s and bin it
    for (i = 0; i < numBlocks; i++)
    {
        uint64_t longestRun = FindLongestRun(bitstreamBuffer, i * blockSize, blockSize);
        int64_t diff = (int64_t)longestRun - (int64_t)expectedMaxInt;

        if (diff < -2)
            observed[0]++;          // bin 0: length < expectedMax - 2
        else if (diff == -2)
            observed[1]++;          // bin 1: length == expectedMax - 2
        else if (diff == -1)
            observed[2]++;          // bin 2: length == expectedMax - 1
        else if (diff == 0)
            observed[3]++;          // bin 3: length == expectedMax
        else if (diff == 1)
            observed[4]++;          // bin 4: length == expectedMax + 1
        else
            observed[5]++;          // bin 5: length > expectedMax + 1
    }

    // Compute expected probabilities using Gumbel-type distribution for longest runs:
    // P(longest run >= k) ~= 1 - exp(-blockSize * 2^(-k))
    // P(longest run == k) ~= exp(-blockSize * 2^(-(k+1))) - exp(-blockSize * 2^(-k))
    {
        // Precompute P(longest run >= k) for relevant k values
        // k ranges from (expectedMaxInt - 2) to (expectedMaxInt + 2)
        double pGeq[LHR_NUM_BINS + 1];  // P(>= expectedMaxInt + j) for various j

        // P(longest run >= k) = 1 - exp(-blockSize * 2^(-k))
        // We need: expectedMaxInt - 2, expectedMaxInt - 1, expectedMaxInt,
        //          expectedMaxInt + 1, expectedMaxInt + 2

        for (j = -2; j <= 3; j++)
        {
            int k = expectedMaxInt + j;
            if (k < 0)
                pGeq[j + 2] = 1.0;
            else
                pGeq[j + 2] = 1.0 - exp(-(double)blockSize * pow(2.0, -(double)k));
        }

        // bin 0: P(length < expectedMax - 2) = 1 - P(>= expectedMax - 2)
        expectedProb[0] = 1.0 - pGeq[0];

        // bin 1: P(length == expectedMax - 2) = P(>= expectedMax - 2) - P(>= expectedMax - 1)
        expectedProb[1] = pGeq[0] - pGeq[1];

        // bin 2: P(length == expectedMax - 1) = P(>= expectedMax - 1) - P(>= expectedMax)
        expectedProb[2] = pGeq[1] - pGeq[2];

        // bin 3: P(length == expectedMax) = P(>= expectedMax) - P(>= expectedMax + 1)
        expectedProb[3] = pGeq[2] - pGeq[3];

        // bin 4: P(length == expectedMax + 1) = P(>= expectedMax + 1) - P(>= expectedMax + 2)
        expectedProb[4] = pGeq[3] - pGeq[4];

        // bin 5: P(length > expectedMax + 1) = P(>= expectedMax + 2)
        expectedProb[5] = pGeq[4];

        // Ensure no negative probabilities
        for (j = 0; j < LHR_NUM_BINS; j++)
        {
            if (expectedProb[j] < 0.0)
                expectedProb[j] = 0.0;
        }

        // Expected counts
        for (j = 0; j < LHR_NUM_BINS; j++)
            expected[j] = (double)numBlocks * expectedProb[j];
    }
    ...
    privateData->probabilityValue = cephes_igamc((double)LHR_DF / 2.0,
                                                  chiSquared / 2.0);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_LongestHeadRunPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_LongestHeadRunPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->blockSize = BLOCK_SIZE_DEFAULT;
...
{
    ((tTestU01_LongestHeadRunPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_LongestHeadRunPrivateData* privData = (tTestU01_LongestHeadRunPrivateData*)testPrivateData;
    bool passed = false;
