// =================================================================================================
//! @file appearance_spacings.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Appearance Spacings test for the STEER framework.
...
#define PROGRAM_NAME        "testu01_appearance_spacings_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "appearance spacings"
#define TEST_DESCRIPTION \
"The Appearance Spacings test is a variant of Maurer's Universal Statistical Test. The \
bitstream is divided into L-bit blocks. For each block value, the number of blocks since \
that value last appeared (the spacing) is recorded. The average log2 of spacings is \
computed over K test blocks (after Q initialization blocks). Under H0, the test statistic \
follows a known distribution. A Z-score is computed and converted to a p-value. This test \
detects generators that produce patterns with anomalous repetition spacings."
#define CONFIGURATION_COUNT         1
#define TESTU01_NAME                "TestU01 Crush Battery"

#define MINIMUM_BITSTREAM_COUNT     1
#define MINIMUM_BITSTREAM_LENGTH    25600
#define MINIMUM_SIGNIFICANCE_LEVEL  0.0
#define MAXIMUM_SIGNIFICANCE_LEVEL  1.0

// Appearance Spacings parameters
#define AS_L                8       // Block length in bits
#define AS_NUM_VALUES       256     // 2^L possible block values
#define AS_Q                640     // Initialization blocks (discarded)
#define AS_K                2560    // Test blocks
#define AS_EXPECTED_MEAN    7.1836  // Expected value of f_n for L=8
#define AS_VARIANCE_COEFF   3.238   // Var ≈ 3.238 / K

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_appearancespacingsprivatedata
{
...
tTestU01_AppearanceSpacingsPrivateData;
...
// =================================================================================================
//  Helper: extract L-bit block value from bitstream at given bit offset
// =================================================================================================
static uint32_t ExtractBlock(uint8_t* buffer, uint64_t bitOffset, int numBits)
{
    uint32_t value = 0;
    int i;
    for (i = 0; i < numBits; i++)
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_AppearanceSpacingsPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    int64_t lastSeen[AS_NUM_VALUES];
    uint64_t totalBlocks;
    uint64_t Q, K;
    double fn = 0.0;
    double expectedMean = AS_EXPECTED_MEAN;
    double variance;
    double zScore;
    uint64_t i;

    // Determine how many blocks we can extract
    totalBlocks = privateData->bitstreamLength / AS_L;
    Q = (totalBlocks > (uint64_t)(AS_Q + AS_K)) ? AS_Q : (totalBlocks / 4);
    K = (totalBlocks > (uint64_t)(AS_Q + AS_K)) ? AS_K : (totalBlocks - Q);

    // Initialize last-seen table to -1 (not seen)
    for (i = 0; i < AS_NUM_VALUES; i++)
        lastSeen[i] = -1;

    // Initialization phase: process Q blocks
    for (i = 0; i < Q; i++)
    {
        uint32_t blockVal = ExtractBlock(bitstreamBuffer, i * AS_L, AS_L);
        lastSeen[blockVal] = (int64_t)i;
    }

    // Test phase: process K blocks, computing sum of log2(spacings)
    fn = 0.0;
    for (i = Q; i < Q + K; i++)
    {
        uint32_t blockVal = ExtractBlock(bitstreamBuffer, i * AS_L, AS_L);
        ...
        if (lastSeen[blockVal] >= 0)
            spacing = (int64_t)i - lastSeen[blockVal];
        else
            spacing = (int64_t)i + 1;  // First appearance: use distance from start

        if (spacing > 0)
            fn += log2((double)spacing);
    ...
    variance = AS_VARIANCE_COEFF / (double)K;
    zScore = (fn - expectedMean) / sqrt(variance);

    // Store chi-squared as Z^2 for reporting consistency
    privateData->chiSquared = zScore * zScore;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_AppearanceSpacingsPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_AppearanceSpacingsPrivateData), (void**)&privData);
...
{
    ((tTestU01_AppearanceSpacingsPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_AppearanceSpacingsPrivateData* privData = (tTestU01_AppearanceSpacingsPrivateData*)testPrivateData;
    bool passed = false;
