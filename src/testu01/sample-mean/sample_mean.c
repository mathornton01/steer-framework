...
//! @brief This file implements the TestU01 Sample Mean test (svaria_SampleMean) for the
//!        STEER framework.
...
#define PROGRAM_NAME        "testu01_sample_mean_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "sample mean"
#define TEST_DESCRIPTION \
"The Sample Mean test (svaria_SampleMean) generates groups of n uniform [0,1) values and \
computes the sample mean of each group. By the Central Limit Theorem, these means are \
approximately normal with mean 0.5 and variance 1/(12n). The test standardizes each \
sample mean and applies the Anderson-Darling goodness-of-fit test against the standard \
normal distribution. It detects deviations in the first moment of groups of consecutive \
generator outputs."
...
#define SMEAN_BITS_PER_VALUE    32      // Bits per uniform value
#define SMEAN_N_DEFAULT         30      // Values per group (for CLT to apply)

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_samplemeanprivatedata
{
...
tTestU01_SampleMeanPrivateData;
...
static double ExtractUniform(uint8_t* buffer, uint64_t bitOffset)
{
    uint32_t value = 0;
    int i;
    for (i = 0; i < SMEAN_BITS_PER_VALUE; i++)
        value = (value << 1) | buffer[bitOffset + i];
    return (double)value / 4294967296.0;
}

// =================================================================================================
//  Helper: standard normal CDF via erfc
// =================================================================================================
static double NormalCDF(double x)
{
    return 0.5 * cephes_erfc(-x / sqrt(2.0));
}
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_SampleMeanPrivateData* privateData,
                ...
                bool* passed)
{
    ...
    numValues = privateData->bitstreamLength / SMEAN_BITS_PER_VALUE;
    ...
    // Standard deviation of sample mean: sqrt(1/(12*n))
    stddev = sqrt(1.0 / (12.0 * (double)groupSize));

    // Allocate array for p-values
    result = STEER_AllocateMemory(numGroups * sizeof(double), (void**)&pvalues);
    if (result != STEER_RESULT_SUCCESS) return result;

    // For each group, compute sample mean, standardize, and convert to p-value
    for (i = 0; i < numGroups; i++)
    {
        ...
        {
            sum += ExtractUniform(bitstreamBuffer,
                                  (i * groupSize + j) * SMEAN_BITS_PER_VALUE);
        }

        mean = sum / (double)groupSize;
        z = (mean - 0.5) / stddev;

        // Convert standard normal z to uniform p-value via CDF
        pvalues[i] = NormalCDF(z);
    }
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_SampleMeanPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_SampleMeanPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->groupSize = SMEAN_N_DEFAULT;
...
{
    ((tTestU01_SampleMeanPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_SampleMeanPrivateData* privData = (tTestU01_SampleMeanPrivateData*)testPrivateData;
    bool passed = false;
