// =================================================================================================
//! @file hamming_independence.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Hamming Independence test (sstring_HammingIndep) for the
//!        STEER framework.
...
#define PROGRAM_NAME        "testu01_hamming_independence_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "hamming independence"
#define TEST_DESCRIPTION \
"The Hamming Independence test (sstring_HammingIndep) divides the bitstream into non-overlapping \
pairs of consecutive blocks. For each pair, the Hamming weights of both blocks are computed. \
Under the null hypothesis, these weights are independent random variables. The test constructs \
a contingency table of weight categories for the two blocks and applies a chi-squared test of \
independence. It detects generators where the weight of one output word depends on the weight \
...
#define HINDEP_BLOCK_SIZE_DEFAULT   32
#define HINDEP_NUM_CATS             5

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_hammingindependenceprivatedata
{
...
tTestU01_HammingIndependencePrivateData;
...
// =================================================================================================
//  Helper: count 1-bits in a block of the bitstream
// =================================================================================================
static uint64_t CountOnesInBlock(uint8_t* buffer, uint64_t bitOffset, uint64_t blockSize)
{
    uint64_t count = 0;
    uint64_t i;
    for (i = 0; i < blockSize; i++)
        count += buffer[bitOffset + i];
    return count;
}

// =================================================================================================
//  Helper: categorize a Hamming weight into one of HINDEP_NUM_CATS bins
// =================================================================================================
static int CategorizeWeight(uint64_t weight, double mean, double sigma)
{
    double w = (double)weight;
    if (w < mean - 1.5 * sigma) return 0;
    if (w < mean - 0.5 * sigma) return 1;
    if (w < mean + 0.5 * sigma) return 2;
    if (w < mean + 1.5 * sigma) return 3;
    return 4;
}
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_HammingIndependencePrivateData* privateData,
                ...
                bool* passed)
{
    ...
    double mean = (double)blockSize / 2.0;
    double sigma = sqrt((double)blockSize / 4.0);
    uint64_t observed[HINDEP_NUM_CATS][HINDEP_NUM_CATS];
    uint64_t rowTotal[HINDEP_NUM_CATS];
    uint64_t colTotal[HINDEP_NUM_CATS];
    double chiSquared = 0.0;
    int df = (HINDEP_NUM_CATS - 1) * (HINDEP_NUM_CATS - 1);
    uint64_t p;
    int r, c;

    memset(observed, 0, sizeof(observed));
    memset(rowTotal, 0, sizeof(rowTotal));
    ...
    {
        uint64_t w1 = CountOnesInBlock(bitstreamBuffer, (2 * p) * blockSize, blockSize);
        uint64_t w2 = CountOnesInBlock(bitstreamBuffer, (2 * p + 1) * blockSize, blockSize);
        int cat1 = CategorizeWeight(w1, mean, sigma);
        int cat2 = CategorizeWeight(w2, mean, sigma);
        observed[cat1][cat2]++;
        rowTotal[cat1]++;
        colTotal[cat2]++;
    }
    ...
    chiSquared = 0.0;
    for (r = 0; r < HINDEP_NUM_CATS; r++)
    {
        for (c = 0; c < HINDEP_NUM_CATS; c++)
        {
            double expected = (double)rowTotal[r] * (double)colTotal[c] / (double)numPairs;
            if (expected > 0.0)
            {
                double diff = (double)observed[r][c] - expected;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_HammingIndependencePrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_HammingIndependencePrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->blockSize = HINDEP_BLOCK_SIZE_DEFAULT;
...
{
    ((tTestU01_HammingIndependencePrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_HammingIndependencePrivateData* privData = (tTestU01_HammingIndependencePrivateData*)testPrivateData;
    bool passed = false;
