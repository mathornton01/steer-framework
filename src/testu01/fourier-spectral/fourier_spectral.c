// =================================================================================================
//! @file fourier_spectral.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Fourier Spectral test (sspectral_Fourier3) for the
//!        STEER framework.
...
#define PROGRAM_NAME        "testu01_fourier_spectral_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "fourier spectral"
#define TEST_DESCRIPTION \
"The Fourier Spectral test (sspectral_Fourier3) maps the bitstream to +1/-1 values and \
computes the discrete Fourier transform. Under the null hypothesis, the squared magnitudes \
of the Fourier coefficients follow a chi-squared distribution with 2 degrees of freedom \
(i.e., exponential). The test counts how many spectral peaks exceed a threshold \
T = sqrt(2n * ln(1/0.05)), where 95% of peaks should fall below T. A normal approximation \
is used to test whether the observed number of peaks below T matches the expected 95%."
...
// Spectral test parameters
#define SPECTRAL_MAX_BITS   8192

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_fourierspectralprivatedata
{
...
tTestU01_FourierSpectralPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_FourierSpectralPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t n = privateData->bitstreamLength;
    uint64_t dftLen = (n > SPECTRAL_MAX_BITS) ? SPECTRAL_MAX_BITS : n;
    uint64_t nFreq = dftLen / 2;
    double threshold2 = 2.0 * (double)dftLen * log(1.0 / 0.05);
    uint64_t nBelow = 0;
    ...
    for (k = 1; k <= nFreq; k++)
    {
        ...
        double si = 0.0;
        uint64_t i;
        double omega = 2.0 * M_PI * (double)k / (double)dftLen;

        for (i = 0; i < dftLen; i++)
        {
            double x = 2.0 * (double)bitstreamBuffer[i] - 1.0;
            sr += x * cos(omega * (double)i);
            si += x * sin(omega * (double)i);
        }
    ...
    expectedBelow = 0.95 * (double)nFreq;
    z = ((double)nBelow - expectedBelow) / sqrt(0.95 * 0.05 * (double)nFreq);
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_FourierSpectralPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_FourierSpectralPrivateData), (void**)&privData);
...
{
    ((tTestU01_FourierSpectralPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_FourierSpectralPrivateData* privData = (tTestU01_FourierSpectralPrivateData*)testPrivateData;
    bool passed = false;
