...
//! @brief This file implements the TestU01 Autocorrelation test (sstring_AutoCor) for the STEER
//!        framework.
...
#define PROGRAM_NAME        "testu01_autocorrelation_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "autocorrelation"
#define TEST_DESCRIPTION \
"The Autocorrelation test (sstring_AutoCor) computes the autocorrelation of the bitstream at a \
given lag d. For each bit position i, it computes the XOR of bit[i] and bit[i+d], then sums these \
XOR values. Under the null hypothesis of independence, this sum follows a binomial distribution \
centered at (n-d)/2 with variance (n-d)/4. The test standardizes this to a z-score and computes a \
two-sided p-value. It detects generators whose output bits have dependencies at specific lag \
distances."
...
// Autocorrelation test parameters
#define LAG_DEFAULT                 1

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_autocorrelationprivatedata
{
...
tTestU01_AutocorrelationPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_AutocorrelationPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t n = privateData->bitstreamLength;
    uint64_t d = privateData->lag;
    uint64_t A = 0;
    uint64_t i;
    double effectiveN;
    double mean;
    ...
    // For each bit position i from 0 to n-d-1, compute XOR = bit[i] ^ bit[i+d]
    // Sum all XOR values
    for (i = 0; i < n - d; i++)
        A += (uint64_t)(bitstreamBuffer[i] ^ bitstreamBuffer[i + d]);

    // Under H0, E[A] = (n-d)/2, Var[A] = (n-d)/4
    effectiveN = (double)(n - d);
    mean = effectiveN / 2.0;
    stddev = sqrt(effectiveN / 4.0);

    // z = (A - (n-d)/2) / sqrt((n-d)/4)
    z = ((double)A - mean) / stddev;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_AutocorrelationPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_AutocorrelationPrivateData), (void**)&privData);
...
{
    ((tTestU01_AutocorrelationPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_AutocorrelationPrivateData* privData = (tTestU01_AutocorrelationPrivateData*)testPrivateData;
    bool passed = false;
