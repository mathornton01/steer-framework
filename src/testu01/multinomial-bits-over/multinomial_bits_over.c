// =================================================================================================
//! @file multinomial_bits_over.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Multinomial Bits Over test for the STEER framework.
...
#define PROGRAM_NAME        "testu01_multinomial_bits_over_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "multinomial bits over"
#define TEST_DESCRIPTION \
"The Multinomial Bits Over test (smultin_MultinomialBitsOver) partitions the bitstream into \
overlapping L-bit tuples and counts the occurrences of each possible L-bit pattern. Under the \
null hypothesis, each of the 2^L patterns should appear with equal probability. A chi-squared \
test compares the observed counts against the expected uniform distribution. This overlapping \
version (using sliding windows) provides more samples and greater statistical power than the \
...
// Multinomial Bits Over parameter defaults
#define TUPLE_SIZE_DEFAULT          4

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_multinomialbitsoverprivatedata
{
...
tTestU01_MultinomialBitsOverPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_MultinomialBitsOverPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint64_t L = privateData->tupleSize;
    uint64_t numPatterns = 1ULL << L;
    uint64_t n = privateData->bitstreamLength;
    uint64_t numTuples = n - L + 1;
    ...
    // Count overlapping tuples
    // First tuple
    val = 0;
    for (j = 0; j < L; j++)
        val = (val << 1) | bitstreamBuffer[j];
    counts[val]++;
    ...
    {
        val = ((val << 1) | bitstreamBuffer[i + L - 1]) & mask;
        counts[val]++;
    }

    // Chi-squared
    expected = (double)numTuples / (double)numPatterns;
    chiSquared = 0.0;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_MultinomialBitsOverPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_MultinomialBitsOverPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        // Set defaults
        privData->tupleSize = TUPLE_SIZE_DEFAULT;
...
{
    ((tTestU01_MultinomialBitsOverPrivateData*)testPrivateData)->report = report;
    return STEER_RESULT_SUCCESS;
}
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_MultinomialBitsOverPrivateData* privData = (tTestU01_MultinomialBitsOverPrivateData*)testPrivateData;
    bool passed = false;
