// =================================================================================================
//! @file collision_over.c
//! @author STEER Framework Contributors
//! @brief This file implements the TestU01 Collision Over test for the STEER framework.
...
#define PROGRAM_NAME        "testu01_collision_over_test"
#define PROGRAM_VERSION     "0.1.0"
#define TEST_NAME           "collision over"
#define TEST_DESCRIPTION \
"The Collision Over test generates n random values in {0,...,k-1} using overlapping \
s-bit windows from the bitstream. It counts collisions, where a value lands on an \
already-occupied cell. Under the null hypothesis of randomness, the expected number \
of collisions and its variance can be computed from the balls-into-bins model. The \
test standardizes the collision count and uses a normal approximation for the P-value."
...
// Collision Over parameter defaults
#define COLL_S_DEFAULT          10      // Bits per window
#define COLL_N_DEFAULT          10000   // Number of balls (values)

// =================================================================================================
//  Private types
// =================================================================================================

typedef struct ttestu01_collisionoverprivatedata
{
...
tTestU01_CollisionOverPrivateData;
...
//  RunTest
// =================================================================================================
int32_t RunTest(tTestU01_CollisionOverPrivateData* privateData,
                uint8_t* bitstreamBuffer,
                bool* passed)
{
    int32_t result = STEER_RESULT_SUCCESS;
    uint8_t* occupied = NULL;
    uint64_t maxValues;
    uint64_t n;
    int32_t collisions = 0;
    double expectedC, varC, p, zScore;
    uint64_t i;
    uint64_t collS = privateData->bitsPerWindow;
    uint64_t collK = (uint64_t)1 << collS;  // 2^bitsPerWindow = number of cells
    uint64_t collN = privateData->numValues;

    // Number of overlapping s-bit windows we can extract
    // Each window starts 1 bit after the previous one
    maxValues = privateData->bitstreamLength - collS + 1;
    n = (maxValues > collN) ? collN : maxValues;

    // Allocate occupancy array
    result = STEER_AllocateMemory(collK * sizeof(uint8_t), (void**)&occupied);
    if (result != STEER_RESULT_SUCCESS)
        return result;

    memset(occupied, 0, collK * sizeof(uint8_t));

    // Generate values using overlapping s-bit windows and count collisions
    collisions = 0;
    for (i = 0; i < n; i++)
    {
        uint32_t value = 0;
        uint64_t b;
        for (b = 0; b < collS; b++)
            value = (value << 1) | bitstreamBuffer[i + b];
    ...
    // Expected collisions: E[C] = n - k*(1 - (1-1/k)^n) ≈ n - k*(1-exp(-n/k))
    p = 1.0 - exp(-(double)n / (double)collK);
    expectedC = (double)n - (double)collK * p;

    // Variance: Var ≈ k*p*(1-p) - n*p*(1-2*p)
    // where p = 1 - exp(-n/k)
    varC = (double)collK * p * (1.0 - p) - (double)n * p * (1.0 - 2.0 * p);

    // Standardize
    zScore = (varC > 0.0) ? ((double)collisions - expectedC) / sqrt(varC) : 0.0;

    privateData->collisionCount = collisions;
    privateData->expectedCollisions = expectedC;
    privateData->zScore = zScore;

    // P-value via complementary error function
    privateData->probabilityValue = erfc(fabs(zScore) / sqrt(2.0));

    *passed = (privateData->probabilityValue >= privateData->significanceLevel);

    STEER_FreeMemory((void**)&occupied);
    return result;
}
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_CollisionOverPrivateData* privData = NULL;

    *testPrivateData = NULL;
    *bufferSizeInBytes = 0;

    result = STEER_AllocateMemory(sizeof(tTestU01_CollisionOverPrivateData), (void**)&privData);
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        privData->bitsPerWindow = COLL_S_DEFAULT;
        privData->numValues = COLL_N_DEFAULT;
        ...
        for (i = 0; i < parameters->count; i++)
        {
            ...
            else if (strcmp(parameters->parameter[i].name, "bits per window") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->bitsPerWindow = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
            else if (strcmp(parameters->parameter[i].name, "num values") == 0)
            {
                result = STEER_GetNativeValue(parameters->parameter[i].dataType,
                                              parameters->parameter[i].value, &nativeValue);
                if (result == STEER_RESULT_SUCCESS)
                {
                    privData->numValues = *((uint64_t*)nativeValue);
                    STEER_FreeMemory(&nativeValue);
                }
            }
...
{
    ((tTestU01_CollisionOverPrivateData*)testPrivateData)->report = report;
...
{
    int32_t result = STEER_RESULT_SUCCESS;
    tTestU01_CollisionOverPrivateData* privData = (tTestU01_CollisionOverPrivateData*)testPrivateData;
    bool passed = false;
    ...
    {
        memset(calculationStr, 0, STEER_STRING_MAX_LENGTH);
        sprintf(calculationStr, "%d", privData->collisionCount);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            "collision count",
    ...
    if (result == STEER_RESULT_SUCCESS)
    {
        ...
        sprintf(calculationStr, STEER_DEFAULT_FLOATING_POINT_STRING_FORMAT,
                STEER_DEFAULT_FLOATING_POINT_PRECISION, privData->expectedCollisions);
        result = STEER_AddCalculationToTest(privData->report, 0, testId,
                                            "expected collisions",
                                            STEER_JSON_VALUE_DOUBLE_PRECISION_FLOATING_POINT,
                                            STEER_JSON_VALUE_DEFAULT_FLOATING_POINT_PRECISION,
